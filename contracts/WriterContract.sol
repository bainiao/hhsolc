...
contract Writer{
    StorageContract immutable public sc;
    constructor(StorageContract _sc){
        sc = _sc;
    }
...
contract BurnerDeployer{
    ...
    function Deploy() public returns(StorageContract sc, address writer){
        // contract nonce starts at 1, and only increment when it creates a contract
        StorageContract scd = StorageContract(address(this).computeAddress(2));
        // first creation happens here using nonce = 1
        writer = address(new Writer(scd));
        // second create hanppens here using nonce = 2
        sc = new StorageContract(writer);
        // sanity check
        require(sc == scd, "false compute of create1 address");
    }
}